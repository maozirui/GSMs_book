function [drho] = rho_partial (X, Y, NN, ds, vx, vy, rho)
...
for k=1:NN % for all real particles
    ...
    for j=1:N
        ...
        dfx(k)=dfx(k)+(vx(L(j))-vx(k))/2.*dSx/area;
        dfy(k)=dfy(k)-(vy(L(j))-vy(k))/2.*dSy/area;
    end
end
%%%%% calculate Drho/Dt in the continuity equation %%%%%%%%%%% 
drho=(-dfx-dfy).*rho; % right hand term of the continuity equation
end